...
def normal_apache(n: int, start: datetime, end: datetime) -> list[dict]:
    endpoints    = {"/": 20, "/index.html": 15, "/about": 10, "/products": 12,
                    "/contact": 8, "/login": 10, "/api/v1/status": 7,
                    "/static/main.css": 10, "/static/app.js": 8}
    ...
    for _ in range(n):
        rows.append({
            ...
            "method":           weighted_choice({"GET": 80, "POST": 15, "PUT": 3, "DELETE": 2}),
...
def brute_force_ssh(n: int, start: datetime) -> list[dict]:
    ...
    """
    common_users = ["root", "admin", "ubuntu", "ec2-user", "pi", "test", "guest", "oracle"]
...
def data_exfiltration(n: int, start: datetime) -> list[dict]:
    ...
    for _ in range(n):
        rows.append({
            ...
            "endpoint":         random.choice(["/update", "/api/sync", "/metrics", "/beacon"]),
            "status_code":      200,
            "bytes_sent":       random.randint(50_000, 500_000),  # large payloads — flag this
...
if __name__ == "__main__":
    ...
    if not 0.0 <= args.anomaly_rate <= 1.0:
        raise ValueError(f"--anomaly-rate must be between 0 and 1, got {args.anomaly_rate}")
    ...
    print(f"[*] Generating {n_normal:,} normal records and {n_anomalous:,} anomalous records...")
